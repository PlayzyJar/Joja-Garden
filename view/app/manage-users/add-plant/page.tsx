...
import { AlertModal } from "@/components/modals/alert-modal"; // Importando seu componente
  ...
  const handleConfirmAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlertState({
        ...
        description: "Não foi possível adicionar a planta. Verifique sua conexão e tente novamente.",
        variant: "danger",
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-screen bg-gray-50/50 pb-10 font-poppins relative">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div>
                ...
                <p className="text-tertiary text-sm mt-1">
                   Jardim de: <span className="font-semibold text-primary bg-primary/10 px-2 py-0.5 rounded ml-1">{targetNome}</span>
                </p>
            </div>
          </div>
              ...
              >
                ...
                <div className="h-44 bg-gray-100 relative overflow-hidden flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                        <Button size="sm" className="opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 bg-white text-primary hover:bg-white hover:text-green-700 font-bold shadow-lg">
                ...
                <div className="p-4 flex flex-col flex-1">
                    <div className="flex justify-between items-start gap-2 mb-1">
                        <h3 className="font-bold text-gray-800 text-lg leading-tight group-hover:text-primary transition-colors">{planta.nome}</h3>
                    </div>
                    <p className="text-xs text-gray-500 italic mb-3">{planta.nome_cientifico}</p>
                ...
                <form onSubmit={handleConfirmAdd} className="p-6 space-y-5">
                    ...
                    <div className="bg-green-50/50 p-4 rounded-xl flex items-center gap-4 border border-green-100/50">
                        <div className="h-12 w-12 rounded-lg bg-white shadow-sm overflow-hidden flex items-center justify-center shrink-0">
                             {selectedPlant.img_url && !imgErrors[selectedPlant.id || 0] ? (
                                <img src={selectedPlant.img_url} alt="" className="w-full h-full object-cover" />
                             ) : (
                                <Leaf className="w-6 h-6 text-green-500" />
                             )}
                        </div>
                        <div>
                            <p className="text-[10px] text-green-600 uppercase font-bold tracking-wider">Adicionando</p>
                            <p className="font-bold text-green-900 text-lg leading-none">{selectedPlant.nome}</p>
                        </div>
                    </div>

                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="apelido" className="text-gray-700 font-medium">Apelido (Opcional)</Label>
                            <Input 
                                id="apelido"
                                value={apelido}
                            ...
                            />
                            <p className="text-[11px] text-gray-400">Nome carinhoso que o usuário verá no painel dele.</p>
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="data" className="text-gray-700 font-medium">Data do Plantio</Label>
                    ...
                    <div className="pt-2 flex gap-3">
                        ...
                        <Button 
                            ...
                            disabled={isSubmitting}
                        >
                            {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <span className="flex items-center gap-2"><Plus size={16}/> Adicionar</span>}
                        </Button>
                    </div>
                </form>
            </div>
      ...
      <AlertModal 
        ...
        loading={false}
        confirmText={alertState.variant === 'success' ? "Concluir" : "Entendido"}
        cancelText={alertState.variant === 'success' ? "Adicionar outra" : "Fechar"}
      />
    </div>
  );
}